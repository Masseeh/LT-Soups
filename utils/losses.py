...
class SigLIPLoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        ll = F.binary_cross_entropy_with_logits(logit, one_hot, reduction='sum')
        return ll/logit.size(0)
...
class VSLoss(nn.Module):
    def __init__(self, cls_num_list, tau=-0.5, gamma=0.15):
        super().__init__()
        cls_num_ratio = cls_num_list / torch.sum(cls_num_list)
        log_cls_num = torch.log(cls_num_ratio)
        
        self.additive = log_cls_num * tau
        self.multi = (cls_num_list / torch.max(cls_num_list)) ** gamma

    def forward(self, logit, target):
        logit_adjusted = logit * self.multi.unsqueeze(0) + self.additive.unsqueeze(0)
        return F.cross_entropy(logit_adjusted, target)
