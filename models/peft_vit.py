...
class ViT_Tuner(nn.Module):
    ...
    def __init__(self, cfg, logger, vit_model, num_classes):
        ...
        if (use_vpt_shallow or use_vpt_deep) and (vpt_len is None):
            vpt_len = 10
            # self._print("Visual prompt length set to {}".format(vpt_len))
        ...
        if use_lora:
            lora_list = nn.ModuleList([
                *[None] * (_start),
                *[nn.ModuleDict({
                    "q": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, alpha=cfg.lora_alpha, dtype=dtype),
                    "v": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, alpha=cfg.lora_alpha, dtype=dtype),
        ...
        if use_lora_mlp:
            lora_mlp_list = nn.ModuleList([
                *[None] * (_start),
                *[nn.ModuleDict({
                    "1": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, alpha=cfg.lora_alpha, out_dim=mlp_in_dim, dtype=dtype),
                    "2": LoRA(in_dim=mlp_in_dim, bottle_dim=adapter_dim, alpha=cfg.lora_alpha, out_dim=emb_dim, dtype=dtype),
                }) for _ in range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
        else:
...
class Peft_ViT(nn.Module):
    def __init__(self, logger, vit_model, use_proj=False):
        super().__init__()
    ...
    def forward(self, x, tuner=None, head=None, return_feature=False):
